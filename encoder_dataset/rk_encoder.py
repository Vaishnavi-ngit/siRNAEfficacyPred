...
for n in range(10):
    ...
    # sirna_sfold_tensor = torch.tensor(sirna_sfold_feat.values, dtype=torch.float)
    # mrna_sfold_tensor = torch.tensor(mrna_sfold_feat.values, dtype=torch.float)
    ...
    def train():
        ...
        for batch in train_loader:
            ...
            # with autocast():  # Enable mixed precision
                # Prepare token_dict from batch
            token_dict = {
                    'siRNA': batch['siRNA'].tokens,   # [num_siRNA_nodes, max_len]
                    'mRNA': batch['mRNA'].tokens      # [num_mRNA_nodes, max_len]
                }

                # Forward pass with both features and tokens
            out = model(
                    x_dict=batch.x_dict,
                    edge_index_dict=batch.edge_index_dict,
                    token_dict=token_dict
                )
            batch_size = batch['interaction'].batch_size
            out = out[:batch_size]
            ground_truth = batch['interaction'].y[:batch_size]
            loss = criterion(out.squeeze(-1), ground_truth)
    ...
    def validate(loader):
        ...
        for batch in loader:
            ...
            batch_size = batch['interaction'].batch_size
            preds.append(out[:batch_size].cpu())
            truths.append(batch['interaction'].y[:batch_size].cpu())
    ...
    #         patience_counter = 0
    #         torch.save(model.state_dict(), f'best256_model_fold{n}_enc.pt')
    #     else:
    #         patience_counter += 1
    ...
    for epoch in range(params["epochs"]):
        ...
        # Make sure tensors are moved to CPU and detached
        val_pred_np = val_pred.squeeze(-1).detach().cpu().numpy()
        val_true_np = val_true.detach().cpu().numpy()

        # Compute MSE Loss
        val_loss = F.mse_loss(val_pred.squeeze(-1), val_true).item()

        # Compute Pearson Correlation Coefficient
        pcc = pearsonr(val_pred_np, val_true_np)[0]  # Only the correlation coefficient, not p-value
        ...
        print(f'Epoch: {epoch:03d}, Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, PCC: {pcc:.4f}')
