...
import math
import re
import E_Utils as utils # Assuming your updated feature functions are here
...
for n in range(10):
    ...
    # Use parameters with defaults for positional encoding
    sirna_length = params.get("sirna_length", 19) # Default to 19 if not in params
    dmodel = params.get("dmodel", 64)           # Default to 64 if not in params
    ...
    sirna_ago = sirna_ago.reindex(sirna_embedding_df.index)

    ## mRNA-AGO2
    mrna_ago = pd.read_csv("RNA_AGO2/mRNA_AGO2_zh.csv",index_col=0)

    mrna_ago = mrna_ago.reindex(mrna_embedding_df.index)
    ...
    # The features of GNN nodes (NOW INCLUDING NEW FEATURES)
    ...
    sirna_pd = pd.concat([
        ...
        # sirna_AU.reindex(sirna_embedding_df.index).fillna(0), # NEW
        # sirna_regional_GC_df.reindex(sirna_embedding_df.index).fillna(0), # NEW
        # sirna_terminal_onehot.reindex(sirna_embedding_df.index).fillna(0), # NEW
    ...
    mrna_pd = pd.concat([
        ...
        #mrna_AU.reindex(mrna_embedding_df.index).fillna(0) # NEW
    ], axis = 1)
    ...
    interaction_pd = pd.concat([
        ...
        #sirna_thermo_asymmetry.reindex(sirna_thermo_feat.index).fillna(0) # NEW
    ],axis=1)
    ...
    # Iterate over the original 'data' to build edges, ensuring nodes exist in final DFs
    ...
    # x_dict and edge_index_dict are implicitly handled by NeighborLoader now
    # x_dict = data_hetero.x_dict
    # edge_index_dict = data_hetero.edge_index_dict


    # Define split indices for NeighborLoader
    # These indices need to correctly map to the 'interaction' nodes that actually exist in the graph
    # Re-generate train/dev/test_idx based on the final interaction_idx,
    # as some interactions might have been implicitly filtered out if their siRNA/mRNA was duplicated and dropped
    ...
    # Update masks based on current interaction_idx
    ...
    for _, row in data.iterrows():
        interaction_name = f"{row['siRNA']}_{row['mRNA']}"
        if interaction_name in interaction_idx: # Ensure interaction node exists in graph
    ...
    def train():
        ...
        for i, batch in enumerate(train_loader):
            # print(f"  Training batch {i+1}/{len(train_loader)}") # For more granular debugging
            optimizer.zero_grad()
    ...
    def test(loader):
        ...
        for i, batch in enumerate(loader):
            # print(f"  Testing batch {i+1}/{len(loader)}") # For more granular debugging
            batch_size = batch['interaction'].num_nodes
            batch_indices = torch.arange(batch_size) # Keep on CPU for collecting
    ...
    model_path = f'model_fold{n}.pt' # Path is now directly in the current directory
