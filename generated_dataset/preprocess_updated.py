...
# Read sirna file
sirna_file = f"full_seq.fas"  # assuming FASTA format for siRNAs
sirna_map = []
...
efiicacy_file = pd.read_csv("full_eff.csv")
...
#     proc = subprocess.Popen(['RNAfold','-p',"--id-prefix=" + id_name], # FIX: Changed to RNAfold
...
#             pos_data_gpu = np.asarray(pos_data) # Convert to CuPy array
            
#             if len(pos_data) < n_components_sirna:
#                 # reduced_data = np.zeros((matrix_size_sirna, 6), dtype=np.float32).get() # CuPy zeros, then .get() to NumPy
#                 reduced_data = np.zeros((matrix_size_sirna, 6), dtype=np.float32) # CuPy zeros, then .get() to NumPy
#             else:
#                 pos_matrix_gpu = np.zeros((matrix_size_sirna, matrix_size_sirna), dtype=np.float32) # CuPy zeros
#                 pos_matrix_gpu[pos_data_gpu[:, 0].astype(np.int32) - 1, pos_data_gpu[:, 1].astype(np.int32) - 1] = pos_data_gpu[:, 2] # Use np.int32
#                 pos_matrix_gpu = pos_matrix_gpu + pos_matrix_gpu.T - np.diag(np.diag(pos_matrix_gpu)) # CuPy operations

#                 # data_matrix_cpu = np.asnumpy(pos_matrix_gpu) # Convert back to NumPy for CPU SVD
#                 data_matrix_cpu = pos_matrix_gpu
...
#         except Exception as e: # Original broad except block
...
#         ['RNAcofold', '-p', f"--id-prefix={id_name}"],
...
# Use CuPy for GPU operations in this block
with cp.cuda.Device(6): # Specifies GPU device 0. Change to 1 if you need device 1.
    for file_path in file_paths_cofold:
        pos_data = np.loadtxt(file_path, usecols=[0, 1, 2])
        pos_data_gpu = np.asarray(pos_data) # Convert to CuPy array

        zero_matrix = np.zeros((matrix_size, matrix_size), dtype=np.float32) # CuPy zeros
        zero_matrix[pos_data_gpu[:, 0].astype(cp.int32) - 1, pos_data_gpu[:, 1].astype(cp.int32) - 1] = pos_data_gpu[:, 2] # Use cp.int32

        mask = np.ones(matrix_size, dtype=cp.bool_) # CuPy ones
        mask[max_sirna_len:max_sirna_len + 2] = False
        pos_matrix_gpu = zero_matrix[mask][:, mask] # Perform slicing on GPU
        pos_matrix_gpu = pos_matrix_gpu + pos_matrix_gpu.T - np.diag(cp.diag(pos_matrix_gpu)) # CuPy operations


        # data_matrix_cpu = cp.asnumpy(pos_matrix_gpu) # Convert back to NumPy for CPU SVD
        data_matrix_cpu = pos_matrix_gpu # Convert back to NumPy for CPU SVD
...
# # --- Load base data ---
# eff_df = pd.read_csv("full_eff.csv")

# # --- Load siRNA sequences ---
# sirna_map = {
#     rec.id: str(rec.seq).upper().replace("T", "U")
#     for rec in SeqIO.parse("full_seq.fas", "fasta")
# }
