...
for n in range(NUM_FOLDS):
    ...
    split_dir = f"siRNA_split_datasets/split1/"
    ...
    train_dataset = EfficacyDataset(data_train, sirna_pd, mrna_pd, interaction_pd)
    dev_dataset   = EfficacyDataset(data_dev, sirna_pd, mrna_pd, interaction_pd)
    ...
    def train(model, dataloader, optimizer, device, use_contrastive=True, lambda_metric=0.1):
        ...
        for batch in dataloader:
            ...
            y_pred, metric_emb = model(mRNA_x, siRNA_x, inter_x)
    ...
    def objective(trial, data_train, data_dev, sirna_pd, mrna_pd, interaction_pd,
              input_dim_mRNA, input_dim_siRNA, input_dim_inter, device):
        # --- Hyperparams to optimize ---
        hidden_dim = trial.suggest_int("hidden_dim", [32, 64, 128])
        embed_dim = trial.suggest_int("embed_dim", [16, 32, 64])
        fusion_dim = trial.suggest_int("fusion_dim", [64, 128])
        mlp_dim = trial.suggest_int("mlp_dim", [32, 64])
        lr = trial.suggest_loguniform("lr", 1e-5, 1e-2)
        lambda_metric = trial.suggest_uniform("lambda_metric", 0.0, 0.3)
        epochs = trial.suggest_categorical("epochs", [5, 10, 15])  # 🔹 multiple epoch options

        # --- Build model ---
        model = EfficacyModel(
        input_dim_mRNA, input_dim_siRNA, input_dim_inter,
        hidden_dim=hidden_dim, embed_dim=embed_dim,
        fusion_dim=fusion_dim, mlp_dim=mlp_dim
    ).to(device)
        ...
        for epoch in range(epochs):
            train(model, train_loader, optimizer, device, use_contrastive=True, lambda_metric=lambda_metric)

        # --- Validate ---
        mse, pcc, spcc, auc, f1, precision, recall = validate(model, dev_loader, device)
    ...
    study.optimize(lambda trial: objective(trial, data_train, data_dev, sirna_pd, mrna_pd, interaction_pd,input_dim_mRNA, input_dim_siRNA, input_dim_inter, device), n_trials=30)
