...
'''class HeteroSAGE(torch.nn.Module):
    def __init__(self, layer_sizes, out_channels):
        super().__init__()
    ...
    def forward(self, x_dict, edge_index_dict):
        for conv in self.convs:
            ...
            #x_dict = {'interaction': F.leaky_relu(x_dict['interaction'])}
...
for n in range(10):
    ...
    test_file = "siRNA_mRNA_all_data.csv"  #  CHANGE THIS PATH
    try:
    ...
    # edges
    source = data_test['siRNA'] + "_" + data_test['mRNA']
    target_siRNA = data_test['siRNA']
    target_mRNA = data_test['mRNA']
    ...
    input_nodes=('interaction', dev_idx),  # This part is correct
    shuffle=False,  # Must be False for validation!
    subgraph_type='induced',  # Ensures proper subgraph sampling
    filter_per_worker=False  # Prevents potential multiprocessing issues
    )'''
    ...
    # 1. Recreate the model with same architecture
    model = HeteroSAGE(
    ...
    print(n)
    #path=f"/content/drive/My Drive/DrugDiscovery/siRNADiscovery/siRNA_split/best_model_fold{n}.pt"
    path=f"best_model_fold{n}.pt"
