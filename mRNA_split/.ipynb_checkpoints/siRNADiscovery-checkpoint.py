...
params = json.load(open("mRNA_param.json", 'r'))
...
for n in range(10):
    ...
    # Read file
    data_train = pd.read_csv("mRNA_split_datasets/split" + str(n) + "/train.csv")
    data_dev = pd.read_csv("mRNA_split_datasets/split" + str(n) + "/dev.csv")
    data_test = pd.read_csv("mRNA_split_datasets/split" + str(n) + "/test.csv")
    ...
    # Co-fold features
    con_feat = pd.read_csv("mRNA_split_preprocess/con_matrix.txt",header=None,index_col=0)
    ...
    # sel-fold features
    ## siRNA
    sirna_sfold_feat = pd.read_csv("mRNA_split_preprocess/self_siRNA_matrix.txt",header=None,index_col=0)

    sirna_sfold_feat = sirna_sfold_feat.reindex(sirna_onehot.index)

    ## mRNA
    mrna_sfold_feat = pd.read_csv("mRNA_split_preprocess/self_mRNA_matrix.txt",header=None,index_col=0)
